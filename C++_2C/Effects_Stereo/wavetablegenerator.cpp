...
//    k > 1, more 'curvy'
//    k > 100, "square-like"
void WavetableGenerator::generateSCurve(float* buffer, int bufSize, float saturation)
{
  float normalizeFactor = 1.0f / atan(saturation);
  for(int i = 0; i < bufSize; i++) {
    ...
    buffer[i] = normalizeFactor * atan(saturation * x);
  }
}
